document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- Referências aos Elementos de Imagem ---
    const imagemFileInput = document.getElementById('imagemFile');
    const previewContainer = document.getElementById('preview-container');
    const imagePreview = document.getElementById('image-preview');
    let imagemParaUpload = null; // Armazenará os dados da imagem para o upload
    ...
    imagemFileInput.addEventListener('change', function(event) {
        ...
        if (file && (file.type === 'image/jpeg' || file.type === 'image/png')) {
            ...
            if (file) {
                Swal.fire('Formato Inválido', 'Por favor, selecione um arquivo de imagem no formato JPG ou PNG.', 'warning');
    ...
    function adicionarLinhaGrupoKit() {
        ...
        divGrupo.innerHTML = `
            ...
            <div class="row g-3">
                <div class="col-md-12"><label class="form-label small">Nome do Grupo</label><input type="text" class="form-control form-control-sm nome-grupo-kit" required></div>
                <div class="col-md-6">
                    ...
                    <select class="form-select form-select-sm tipo-selecao-grupo-kit" required>
                        ...
                        <option value="QUANTIDADE_TOTAL">Quantidade Total</option>
                    </select>
                </div>
        ...
        adicionarLinhaOpcaoKit(divGrupo.querySelector('.lista-opcoes-kit')); // Adiciona uma opção inicial
    }
    ...
    formProduto.addEventListener('submit', async function (e) {
        ...
        const produto = {
            ...
            qtdeMax: parseInt(document.getElementById('qtdeMax').value) || 10,
            ...
            gruposKit: gruposKit,
            receita: [] // Lógica de receita pode ser adicionada aqui se necessário
        };
        ...
        try {
            ...
            if (imagemParaUpload) {
                const responseImagem = await fetch(`/api/produtos/${produtoId}/imagem`, {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        imagemBase64: imagemParaUpload.base64,
                        imagemTipo: imagemParaUpload.tipo
                    })
                });
                 if (!responseImagem.ok) {
                    throw new Error('Produto salvo, mas falha ao enviar imagem.');
                }
            }

            Swal.fire('Sucesso!', `Produto "${produtoCriado.nome}" cadastrado!`, 'success');
            formProduto.reset();
            previewContainer.style.display = 'none';
            imagePreview.src = '#';
            imagemParaUpload = null;
